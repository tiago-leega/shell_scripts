...
####################################################################################################
#  Nome do Arquivo   : SPROCE_VALIDA_REMESSA_DOMICILIO_0001.sh                                     #
...
f_NOM_ARQ_SRC=${v_NOM_DIR_FILE_SRC}"CAD_BCO_DOMIC_TX_"${p_CD_BANDEIRA}${p_CD_CREDENCIADORA}"_*.[Tt][Xx][Tt]"

#Definicao do nome do arquivo Log
f_NOM_ARQ_LOG=${v_NOM_DIR_FILE_TGT}"AO_0040_EPROPRCEXT_VALIDA_REMESSA_DOMICILIO_CRDE_01_01_"${p_CD_BANDEIRA}${p_CD_CREDENCIADORA}"_"${v_DT_EXEC}"_LOG.txt"

#Definicao do nome do arquivo de Retorno
f_NOM_ARQ_RET=${v_NOM_DIR_FILE_RET}"CAD_BCO_DOMIC_RX_"${p_CD_BANDEIRA}${p_CD_CREDENCIADORA}_${v_DT}".txt"

#Definicao do nome do arquivo que contem somente o Header
f_NOM_ARQ_HEADER=${v_NOM_DIR_FILE_TGT}"AB_0040_EPROPRCEXT_VALIDA_REMESSA_DOMICILIO_CRDE_01_01_"${p_CD_BANDEIRA}${p_CD_CREDENCIADORA}"_E0.txt"

#Definicao do nome do arquivo que contem somente o Trailer
f_NOM_ARQ_TRAILER=${v_NOM_DIR_FILE_TGT}"AB_0040_EPROPRCEXT_VALIDA_REMESSA_DOMICILIO_CRDE_01_01_"${p_CD_BANDEIRA}${p_CD_CREDENCIADORA}"_E9.txt"

#Definicao do nome dos FileWatcher
v_NOM_ARQ_VALIDO=${v_NOM_DIR_FILE_TGT}"AO_0040_EPROPRCEXT_VALIDA_REMESSA_DOMICILIO_CRDE_01_01_"${p_CD_BANDEIRA}${p_CD_CREDENCIADORA}"_VALIDO"
v_NOM_ARQ_INVALIDO=${v_NOM_DIR_FILE_TGT}"AO_0040_EPROPRCEXT_VALIDA_REMESSA_DOMICILIO_CRDE_01_01_"${p_CD_BANDEIRA}${p_CD_CREDENCIADORA}"_INVALIDO"
...
# Remove logs antigos                        #
##############################################
find ${v_NOM_DIR_FILE_TGT}AO_0040_EPROPRCEXT_VALIDA_REMESSA_DOMICILIO_CRDE_01_01_${p_CD_BANDEIRA}${p_CD_CREDENCIADORA}*LOG.txt -mtime +${v_DIAS_RETENCAO} -exec rm -f {} \;
...
v_QT_REG_E1_REJ="0000000"
        ...
        echo "${v_TP_REGISTRO_RET}${p_CD_BANDEIRA_RET}${v_CD_CREDENCIADORA_RET}${v_CD_IDENT_ENVIO_RET}${v_NU_REMESSA_RET}${v_DT_ARQUIVO_RET}${v_STATUS_PROC_RET}${v_NUM_REG_INVALID_RET}${v_QT_REG_E1_ACP}${v_QT_REG_E1_REJ}" >> ${f_NOM_ARQ_RET}
}
...
v_NU_OCORRENCIA_HEADER="$2"
        ...
        then
                printf "%24s\n" "$v_HEADER"  >> $f_NOM_ARQ_RET
        else
                v_NU_OCORRENCIA_HEADER=`printf "%024s" "${v_NU_OCORRENCIA_HEADER}"`
                v_HEADER_SEM_OCORRENCIAS=`printf "%24s\n" "$v_HEADER" | cut -c1-24`
                printf "${v_HEADER_SEM_OCORRENCIAS}${v_NU_OCORRENCIA_HEADER}\n" >> $f_NOM_ARQ_RET
        fi
}
...
v_NU_OCORRENCIA_TRAILER=$2
        ...
        then
                ...
                v_TRAILER_SEM_OCORRENCIAS=`echo $v_TRAILER | cut -c1-9`
...
# Inicio do processo                                      #
###########################################################
FNC_Log "### ALERTA: Inicio do Processo: SPROCE_VALIDA_REMESSA_DOMICILIO_CRDE_0001.sh -> Bandeira: ${p_CD_BANDEIRA} - Credenciadora: ${p_CD_CREDENCIADORA} <- ###"

if [ "${p_CD_BANDEIRA}" == "" -o "${p_CD_CREDENCIADORA}" == "" ]
then
	FNC_Log "FALHA : Parametro Bandeira ou Credenciadora nao informados."
	FNC_Log "### ALERTA: Termino do Processo: SPROCE_VALIDA_REMESSA_DOMICILIO_CRDE_0001.sh -> Bandeira: ${p_CD_BANDEIRA} - Credenciadora: ${p_CD_CREDENCIADORA} <- ###"
...
#verifica se tipo de registro diferente de E0, E1, E9
...
do
        ...
        if [ "$v_TP_REGISTRO" != "E0" -a "$v_TP_REGISTRO" != "E1" -a "$v_TP_REGISTRO" != "E9" ]
        then
                v_TP_STATUS_PROC_ARQ=3
...
v_FIRST_LINE=`head -1 ${v_NOM_ARQ_FINAL}`
v_FIRST_LINE="`printf "${v_FIRST_LINE}" | cut -c 1-9`2`printf "${v_FIRST_LINE}\n" | cut -c 11-`\n"
...
v_DT_ARQUIVO=`echo $v_FIRST_LINE | cut -c 16-23`
...
#        FNC_Arq_Retorno $v_TP_STATUS_PROC_ARQ $v_NU_REG_SRC $vf_NU_RMSA ${vf_DT_CICLO}
...
then
		...
		echo `echo ${v_FIRST_LINE} | cut -c 1-48`"000000001" >> ${f_NOM_ARQ_HEADER}
		
		#Gera o arquivo com Trailer
		v_NU_REG_SRC_T=`printf "%09s" ${v_NU_REG_SRC_T}`
		echo `echo ${v_LAST_LINE} | cut -c 1-33`"${v_NU_REG_SRC_T}" >> ${f_NOM_ARQ_TRAILER}
