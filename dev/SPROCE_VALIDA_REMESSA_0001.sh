...
# CARLOS GABRIEL  | 16/09/2016 | ACRESCENTADO _YYYYmmDD-HHMMSS NA VARIAVEL f_NOM_ARQ_RET           #
# CARLOS GABRIEL  | 16/09/2016 | EXCLUIR LOG COM MAIS DE 7 DIAS                                    #
# TIAGO XAVIER    | 13/11/2018 | AJUSTE PARA CONTEMPLAR TIPO DE REGISTRO E4 NO ARQUIVO             #
#ADRIANA DE ASSIS | 05/01/2019 | AJUSTE PARA CONTEMPLAR CADASTRO ON LINE (LINHAS 281 A 318)        #
####################################################################################################
...
f_NOM_ARQ_TRAILER=${v_NOM_DIR_FILE_TGT}"AB_0013_EPROPRCEXT_VALIDA_REMESSA_01_01_"${p_CD_BANDEIRA}${p_CD_CREDENCIADORA}"_E9.txt"

#Definicao do nome dos FileWatcher
v_NOM_ARQ_VALIDO=${v_NOM_DIR_FILE_TGT}"AO_0013_EPROPRCEXT_VALIDA-REMESSA_01_01_"${p_CD_BANDEIRA}${p_CD_CREDENCIADORA}"_VALIDO"
v_NOM_ARQ_INVALIDO=${v_NOM_DIR_FILE_TGT}"AO_0013_EPROPRCEXT_VALIDA-REMESSA_01_01_"${p_CD_BANDEIRA}${p_CD_CREDENCIADORA}"_INVALIDO"
...
# Remove logs antigos                        #
##############################################
find ${v_NOM_DIR_FILE_TGT}AO_0013_EPROPRCEXT_VALIDA_REMESSA_01_01_${p_CD_BANDEIRA}${p_CD_CREDENCIADORA}*LOG.txt -mtime +${v_DIAS_RETENCAO} -exec rm -f {} \;
...
vf_CD_MSG=`cat ${f_NOM_ARQ_EXT_0011} | cut -d '|' -f4`
...
v_QT_REG_E1_ACP=0000000
v_QT_REG_E1_REJ=0000000
v_QT_REG_E2_ACP=0000000
v_QT_REG_E2_REJ=0000000
v_QT_REG_E3_ACP=0000000
v_QT_REG_E3_REJ=0000000
v_QT_REG_E4_ACP=0000000
v_QT_REG_E4_REJ=0000000

        echo "${v_TP_REGISTRO_RET}${p_CD_BANDEIRA_RET}${v_CD_CREDENCIADORA_RET}${v_CD_IDENT_ENVIO_RET}${v_NU_REMESSA_RET}${v_DT_ARQUIVO_RET}${v_STATUS_PROC_RET}${v_NUM_REG_INVALID_RET}${v_QT_REG_E1_ACP}${v_QT_REG_E1_REJ}${v_QT_REG_E2_ACP}${v_QT_REG_E2_REJ}${v_QT_REG_E3_ACP}${v_QT_REG_E3_REJ}${v_QT_REG_E4_ACP}${v_QT_REG_E4_REJ}" >> ${f_NOM_ARQ_RET}
}
...
FNC_Log "### ALERTA: Inicio do Processo: SPROCE_VALIDA_REMESSA_0001.sh -> Bandeira: ${p_CD_BANDEIRA} - Credenciadora: ${p_CD_CREDENCIADORA} <- ###"
...
v_FLAG_CAD_ONLINE=`head -1 ${f_NOM_ARQ_EXT_0011}|cut -d "|" -f5`
if [ "${v_FLAG_CAD_ONLINE}" == 'S' ]
then
	v_NOM_ARQ_FINAL=`head -1 ${v_NOM_DIR_FILE_TGT}AO_0012_CAD_ONLINE_0071606_LST.txt`
	FNC_Log "ALERTA:Credenciadora habilitada para Cadastro OnLine"
	FNC_Log "Arquivo utilizado:"${v_NOM_ARQ_FINAL}
...
v_NOME_ARQ_MSG="AO_MENSAGERIA_EC_"${p_CD_BANDEIRA}${p_CD_CREDENCIADORA}
v_TP_REGISTRO=`echo $v_FIRST_LINE | cut -c1-2`


if [ "${v_FIRST_LINE}" == "" -o "$v_TP_REGISTRO" != "E0"  ]
	then
		echo "ARQUIVO SEM E0" > ${v_NOM_DIR_FILE_TGT}${v_NOME_ARQ_MSG}

	else
		v_MENSAGERIA=`echo $v_FIRST_LINE | cut -c1-2`"|"`echo $v_FIRST_LINE | cut -c49-51`
		echo ${v_MENSAGERIA}>${v_NOM_DIR_FILE_TGT}${v_NOME_ARQ_MSG}
...
v_TP_REGISTRO=`echo $v_FIRST_LINE | cut -c1-2`
v_NU_REG_SRC=00001
v_TP_REGISTRO_E0=${v_TP_REGISTRO}
...
v_LAST_LINE=$(tail -1 ${v_NOM_ARQ_FINAL} | cut -c1-23)"000000000000000000000000"$(tail -1 ${v_NOM_ARQ_FINAL} | cut -c48-54)
v_NU_REG_SRC_T=`wc -l ${v_NOM_ARQ_FINAL} | awk '{ printf("%05d\n", $1) }'`
v_TP_REGISTRO=`echo $v_LAST_LINE | cut -c1-2`
v_TP_REGISTRO_E9=${v_TP_REGISTRO}
...
#verifica se tipo de registro diferente de E0, E1, E2, E3,E4, E9
...
do
        ...
        if [ "$v_TP_REGISTRO" != "E0" -a "$v_TP_REGISTRO" != "E1" -a "$v_TP_REGISTRO" != "E2" -a "$v_TP_REGISTRO" != "E3" -a "$v_TP_REGISTRO" != "E4" -a "$v_TP_REGISTRO" != "E9" ]
        then
                v_TP_STATUS_PROC_ARQ=3
...
then
        ...
        v_FIRST_LINE=`echo "${v_FIRST_LINE}" | cut -c 1-9`"2"`echo "${v_FIRST_LINE}" | cut -c 11-51`
        v_TP_STATUS_PROC_ARQ=4
        FNC_Log "ALERTA: Arquivo sem movimento - Status de Geracao de Arquivo=1!"
        FNC_Arq_Retorno ${v_TP_STATUS_PROC_ARQ} ${v_NU_REG_SRC} ${vf_NU_RMSA}

	FNC_Gera_Header "${v_FIRST_LINE}"
       	FNC_Gera_Trailer $v_LAST_LINE
		FNC_Move_ArqFinal_Processado ${v_NOM_ARQ_FINAL}

		FNC_Log "### ALERTA: Termino do Processo: SPROCE_VALIDA_REMESSA_0001.sh -> Bandeira: ${p_CD_BANDEIRA} - Credenciadora: ${p_CD_CREDENCIADORA} <- ###"

		#Gera o FileWatcher Invalido
		echo "REMESSA SEM MOVIMENTO" >> ${v_NOM_ARQ_INVALIDO}
        exit 99
elif [ "$v_TP_STATUS" != "0" ]
then
...
elif [ "$v_TP_STATUS" == "0" -a $(wc -l ${v_NOM_ARQ_FINAL} | awk '{print $1}') == 2 -a ${v_TP_REGISTRO_E0} == "E0" -a ${v_TP_REGISTRO_E9} == "E9" ]
then
		v_NU_OCORRENCIA_HEADER=${v_NU_OCORRENCIA_HEADER}"0719"
		FNC_Log "FALHA : Foi enviado apenas E0 e E9 para o arquivo indicado com  movimento no campo Status de geracao do arquivo !"
else
...
v_NUM=`echo ${v_DT_ARQUIVO} | grep "^[1234567890]*$"`
if [ "${v_NUM}" == "" ]
then
...
#Valida Mensageria do Arquivo
v_MENSAGERIA=`echo "${v_FIRST_LINE}" | cut -c 49-51`
v_NUM_MENS=`echo ${v_MENSAGERIA} | grep "^[1234567890]*$"`

if [ "${v_NUM_MENS}" == "" ]
then
	v_NU_OCORRENCIA_HEADER=${v_NU_OCORRENCIA_HEADER}"0700"
	FNC_Log "FALHA : Mensageria n�o Informada ou n�o numerica. Registro rejeitado!"
else

	if [ "${v_MENSAGERIA}" != "${vf_CD_MSG}" ]
	then
       		v_NU_OCORRENCIA_HEADER=${v_NU_OCORRENCIA_HEADER}"0700"
       		FNC_Log "FALHA : Numero da Mensageria do Arquivo Enviado '${v_MENSAGERIA}' nao cadastrado na base!"
	else
       		FNC_Log "ALERTA: Validacao do Numero da Mensageria do Arquivo Enviado realizado com sucesso!"
	fi
...
v_QT_REGISTRO_E4_TRAILER=`echo $v_LAST_LINE | cut -c 48-54`
v_TP_STATUS_PROC_ARQ=2
...
#Valida se a quantidade de registros do tipo E4 no arquivo é igual ao informado no trailer
v_QT_REGISTRO_E4_ARQ=`cat ${v_NOM_ARQ_FINAL} | cut -c 1-2 | egrep 'E4' | wc -l`
v_QT_REGISTRO_E4_ARQ=`printf "%07s" $v_QT_REGISTRO_E4_ARQ`
if [ "$v_QT_REGISTRO_E4_TRAILER" != "$v_QT_REGISTRO_E4_ARQ" ]
then
        v_NU_OCORRENCIA_TRAILER=${v_NU_OCORRENCIA_TRAILER}"0708"
        FNC_Log "FALHA : Quantidade de registros informado para o tipo E4 diferente!"
else
        FNC_Log "ALERTA: Validacao do Numero do Registro informado no Trailer para o tipo E4 realizado com sucesso!"
fi
...
then
        ...
        FNC_Arq_Retorno ${v_TP_STATUS_PROC_ARQ} ${v_NU_REG_SRC} ${vf_NU_RMSA}
#aJUSTE 17/10
	v_NU_MSG_AUX=`echo "${v_FIRST_LINE}" | cut -c 49-51`
        FNC_Gera_Header "${v_FIRST_LINE}" "${v_NU_OCORRENCIA_HEADER}" "${v_NU_MSG_AUX}" #`echo "${v_FIRST_LINE}" | cut -c 49-51`
		...
		#Caso nao seja encontrado problemas de Status de processamento 2,3 e 4, sera gerado separadamente arquivo de header e trailer para ser consumido
		#no job wf_0019_EPROPRCEXT_RETORNO_01

		#Gera o arquivo com Trailer
		# Adicionando o codigo 2 no campo 10 para identificar a origem do arquivo - 20170124
		echo `echo ${v_FIRST_LINE} | cut -c 1-9`2`echo ${v_FIRST_LINE} | cut -c 11-51`"000000001" >> ${f_NOM_ARQ_HEADER}

		#Gera o arquivo com Trailer
		v_NU_REG_SRC_T=`printf "%09s" ${v_NU_REG_SRC_T}`
		echo `echo ${v_LAST_LINE} | cut -c 1-54`"${v_NU_REG_SRC_T}" >> ${f_NOM_ARQ_TRAILER}
