...
vf_CD_MSG=`cat ${f_NOM_ARQ_EXT_0026} | cut -d '|' -f4`
...
v_MENSAGERIA="$3"
        ...
        then
                ...
                echo "${v_HEADER_SEM_OCORRENCIAS}${v_NU_OCORRENCIA_HEADER}${v_MENSAGERIA}" >> $f_NOM_ARQ_RET
...
#Verifica Mensageria do arquivo
v_NOM_ARQ_FINAL=`ls -lt ${f_NOM_ARQ_SRC} | head -1 | awk '{print $9}'`
v_FIRST_LINE=`head -1 ${v_NOM_ARQ_FINAL}`
v_NOME_ARQ_MSG="AO_MENSAGERIA_BATIMENTO_"${p_CD_BANDEIRA}${p_CD_CREDENCIADORA}
...
then
        v_TP_STATUS_PROC_ARQ=2
        FNC_Arq_Retorno ${v_TP_STATUS_PROC_ARQ} ${v_NU_REG_SRC} ${vf_NU_RMSA}
        FNC_Gera_Header "${v_FIRST_LINE}" "${v_NU_OCORRENCIA_HEADER}" "${v_MENSAGERIA}"
